...
const DivWithDropdown: React.FC<DivWithDropdownProps> = ({
  ...
  const { primaryValue } = determineQuestionType(textValue);  const handleTypeSelect = (type: string) => {
...
setIsOpen(false);
  ...
  };  const dropdownOptions = ["Text", "Paragraph", "Email", "Radio", "Number", "Date"];  return (
    <div className={flex items-center space-x-8 w-full relative ${isFollowUp ? "ml-0" : ""}}>
      ...
      <div
        className={relative w-full max-w-lg h-36 rounded-xl shadow-lg flex flex-col items-center justify-center text-lg font-semibold p-6 z-10 transform transition-all duration-300 hover:shadow-xl ${           isDarkMode             ? "bg-gradient-to-br from-gray-700 to-gray-800 text-teal-200"             : "bg-gradient-to-br from-teal-100 to-cyan-100 text-teal-900"         }}
      >
        <div className="relative w-full flex items-center">
          ...
          <input
            ...
            className={px-3 py-2 text-sm bg-transparent w-1/2 relative z-10 top-[-10px] max-w-full focus:outline-none transition-all duration-300 ${               isDarkMode                 ? "border-b border-teal-400 text-teal-200 placeholder-teal-300/70 focus:border-cyan-400"                 : "border-b border-teal-400 text-teal-800 placeholder-teal-400/70 focus:border-cyan-500"             }}
...
const Questionnaire_Level3 = () => {
  ...
  const feedbackId = useRef(0);  const followUpQuestions = [
...
setScoredQuestions(prev => ({
  ...
  }, [uniqueQuestions, enhancedDetermineQuestionType, scoredQuestions, updateScore]);  const scoreRequiredStatus = useCallback((index: number, isRequired: boolean) => {
    if (isRequired) {
  ...
  }, [updateScore, scoredQuestions]);  const checkForBonus = useCallback(() => {
...
if (allCorrect) {
  ...
  }, [uniqueQuestions, selectedTypes, requiredQuestions, enhancedDetermineQuestionType, bonusAwarded, updateScore]);  useEffect(() => {
...
setBonusAwarded(false);
  ...
  }, [highlightedTexts, setSelectedTypes, setEditedQuestions, setRequiredQuestions, enhancedDetermineQuestionType]);  useEffect(() => {
    checkForBonus();
  }, [selectedTypes, requiredQuestions, checkForBonus]);  const handleTypeChange = (index: number, type: string) => {
...
if (newTexts[index] === primaryValue || newTexts[index] === "No text selected") {
  ...
  };  const storedLevel = sessionStorage.getItem("level") ?? "none";  return (
    <div
      className={min-h-screen flex flex-col font-sans relative transition-all duration-500 ${         isDarkMode           ? "bg-gradient-to-br from-gray-800 via-gray-900 to-black"           : "bg-gradient-to-br from-indigo-50 via-teal-50 to-pink-50"       }}
    >
      <Navbar 
        level={storedLevel} 
      ...
      />
      <div
        className={absolute top-16 left-6 w-40 h-12 rounded-xl shadow-lg flex items-center justify-center text-sm font-semibold z-20 ${           isDarkMode             ? "bg-gradient-to-r from-gray-700 to-gray-800 text-teal-200"             : "bg-gradient-to-r from-teal-200 to-cyan-200 text-teal-900"         }}
      >
        <div className="relative">
          Score: {totalScore}
          {scoreFeedback && (
            <div 
              ...
              className={absolute -top-6 right-0 font-bold text-lg ${                 scoreFeedback.points > 0                    ? "text-emerald-400"                    : "text-rose-500"               } animate-[float-up_1.5s_ease-out_forwards]}
            >
